...
import caspvale.caspsuporte.atendimento.api.assembler.ProblemasAssembler;
...
import caspvale.caspsuporte.atendimento.domain.service.ProblemasService;
...
public class ProblemasController {
 private final ProblemasAssembler problemasAssembler;
    ...
    @PutMapping("/{iSistema}")
    public ResponseEntity editar(@Valid @RequestBody ProblemasModel mudel, @PathVariable Integer iSistema) {
        permissoes.restritoAoCliente();
        CaspProblemas caspProblemaAtual = problemasService.buscarOuFalhar(iSistema);
        if (mudel.getIProblema()== null || caspProblemaAtual.getIProblema()!= iSistema) {
            throw new NegocioException("Conteúdo difere do item selecionado!");
        }
        CaspProblemas caspProblemaEditado = problemasAssembler.toEntity(mudel);
